...
class ObjectsOnRoadProcessor(object):
    ...
    def selfdriving_and_checkobstacle(self):
        ...
        if dis < self.distance_range:             #Check if the target is in diatance range
            ...
            if dis < (self.distance_stay-0.1) : #Check if the target is too close, if so, the car move back to keep distance at distance_stay
                ...
                #turn.ultra_turn(470)
                #dis2 = ultra_dp.checkdist()
                #if (dis2 <self.distance_stay):
                #    turn.left()
                #turn.ultra_turn(140)
                #dis2 = ultra_dp.checkdist()
                #    turn.right()    
                ...
                motor.motor_right(self.status,self.forward,self.right_spd)            #If the target is at distance, then the car stay still
                #motor.motorStop()
            #led.both_off()
            #led.yellow()
        else:
    ...
    def control_car(self, objects):
        ...
        #self.selfdriving_and_checkobstacle()
        
        for obj in objects:
            obj_label = self.labels[obj.label_id]
            logging.debug('obj_label, %s'%obj_label)
