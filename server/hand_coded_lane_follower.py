...
class HandCodedLaneFollower(object):
    ...
    def steer(self, frame, lane_lines):
        ...
        if self.car is not None:
            ...
            print('self.curr_steering_angle %s'%self.curr_steering_angle)
            
            #angle=370+85*(int(self.curr_steering_angle)-90)/45
            #print('angle %s'%angle)
            #turn.turn_ang(int(angle))
            motor.motor_right(status,forward,right_spd)
            time.sleep(1)
            
            #if status_middle == 1 and status_right == 1 and status_left == 1:
                #turn.turn_ang(335+(90-int(old_steering_angle)))  #370
            #    turn.center()
            #    motor.motor_right(status,backward,right_spd)
    ...
    if lines is not None:
        ...
        for line in lines:
            for x1, y1, x2, y2 in line:
                circle_x=circle_x+x2
                circle_y=circle_y+y2
                print(x1,x2,y1,y2)
                cv2.line(line_image, (x1, y1), (x2, y2), line_color, line_width)
           
        #line_image = cv2.circle(line_image, ((int(circle_x/2)-50),(int(circle_y/2)-60)), 50, color, thickness)         
