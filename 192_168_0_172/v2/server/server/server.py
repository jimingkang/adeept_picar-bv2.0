            ...
            if len(cnts) > 0:
                ...
                if radius > 10: #TODO: make min size a setting
                    ...
                    if Y < 230:
                        ...
                        camera_turn(vtr_mid_orig) #TODO change to tilt
...
def run():
    ...
    while True: #??? only exits from breaks, seems wrong
        ...
        try: # try to ping to see if there is wifi
            ...
            #??? this only checks if internet not wifi only?
        ...
        if wifi_status == 1:
            ...
            #??? now assuming no ap-hotspot, but never explained
            led.both_off()
            led.blue()
